...
class RemezSuite extends FlatSpec with Matchers {
    ...
    // Test 1.
    "The Remez class" should f"find the coeffs for test 1 (params: f(x)=sin(x), low=0.0, hi=2^-10, order=2)" in {
    ...
    // Test 2.
    it should f"find the coeffs for test 2 (params: f(x)=exp(x), low=0.0, hi=2^-10, order=2)" in {
